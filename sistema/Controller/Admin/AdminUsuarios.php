...
class AdminUsuarios extends AdminController
{
    ...
    {
        // verificar permissão de usuario
        if($this->usuario->tipo_usuario_id != 1){
            $this->mensagem->erro("Sem permissão de acesso")->flash();
        ...
        echo $this->template->renderizar('usuarios/listar.html', [
            'usuarios' => $usuarios->busca()->resultado(true),
            'tipo_usuarios' => $tipo_usuarios,
            'total' => [
                'usuarios' => $usuarios->busca('tipo_usuario_id != 1')->total(),
                'usuariosAtivo' => $usuarios->busca('tipo_usuario_id != 1 AND status = 1')->total(),
                'usuariosInativo' => $usuarios->busca('tipo_usuario_id != 1 AND status = 2')->total(),
                'admin' => $usuarios->busca('tipo_usuario_id = 1')->total(),
                'adminAtivo' => $usuarios->busca('tipo_usuario_id = 1 AND status = 1')->total(),
                'adminInativo' => $usuarios->busca('tipo_usuario_id = 1 AND status = 2')->total()
            ]
        ]);
    }
